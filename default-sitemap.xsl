...
>
	...
	<div id="content-head">
		...
		<p>Dibuat oleh <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank">All in One SEO</a>, ini adalah Sitemap XML, dimaksudkan untuk dikonsumsi oleh mesin pencari seperti Google atau Bing. </p>
		<p>
			Anda dapat menemukan informasi lebih lanjut tentang sitemap XML di <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener">sitemaps.org</a>.		</p>
		<xsl:if test="$amountOfURLs &gt; 0">
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						Sitemap berisi						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
					...
					<xsl:otherwise>
						Indeks sitemap berisi						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								sitemap							</xsl:when>
							<xsl:otherwise>
				...
				and was generated on 28 November 2023 at 7:52 am			</p>
		</xsl:if>
	</div>
</xsl:template>
